...
function* download(pkg, options) {
  ...
  while (count < 3) {
    try {
      ...
      if (err.name === 'ShasumNotMatchError' || err.status >= 500) {
        count++;
        options.console.warn(`[${pkg.name}@${pkg.version}] download error: %s, fail count: %s`,
          err.message, count);
  ...
  if (binaryMirror) {
    ...
    } else if (pkg.name === 'flow-bin') {
      // use mirror url instead
      const indexFilepath = path.join(ungzipDir, 'lib/index.js');
      if (yield fs.exists(indexFilepath)) {
        const content = yield fs.readFile(indexFilepath, 'utf8');
        yield fs.writeFile(indexFilepath,
          content.replace('https://github.com/facebook/flow/releases/download/v', binaryMirror.host));
      }
    }
  }
  ...
  return function(callback) {
    ...
    readstream.on('end', () => {
      ...
      if (realShasum !== shasum) {
        const err = new Error(`${pkg.name}@${pkg.version} sha1:${realShasum} not equal to ${shasum}`);
